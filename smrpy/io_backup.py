...
def get_ld_matrix(G, snp_list, chrom=None):
    ...
    genotypes = G_sub.isel(variant=idx).values.astype(float)
    
    # Orient to (Samples x Variants)
...
def get_ld_vector(G, target_snp, snp_list, chrom=None):
    ...
    # Extract list genotypes
    G_list = G_sub.isel(variant=idx_list).values.astype(float)
    if G_sub.dims[0] == 'variant': G_list = G_list.T
